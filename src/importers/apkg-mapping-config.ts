...
import type { DeckSettings, FSRSCard, ReviewLog } from "../data/types";
 ...
 * 数据映射工具函数
 */
export function convertAnkiTimestamp(ankiTimestamp: number, isMilliseconds = false): string {
 ...
 */
export function mapCardType(ankiModel: AnkiModel): CardType {
	if (ankiModel.type === 1) {
	...
	// 检查是否包含选择题特征
	const hasMultipleChoice = ankiModel.tmpls.some(
		(tmpl) =>
			tmpl.qfmt.includes("{{type:") || tmpl.qfmt.includes("choice") || tmpl.afmt.includes("choice")
	);
	...
	// 检查是否包含代码特征
	const hasCode =
		ankiModel.tmpls.some((tmpl) => tmpl.qfmt.includes("<code>") || tmpl.qfmt.includes("{{code")) ||
		ankiModel.css.includes("code") ||
		ankiModel.css.includes("pre");
 ...
 */
export function mapCardState(ankiCardType: number): CardState {
	switch (ankiCardType) {
		case 0:
 ...
 */
export function mapRating(ankiEase: number): Rating {
	switch (ankiEase) {
		case 1:
			return Rating.Again;
		case 2:
 ...
 * 解析Anki字段内容（0x1f分隔）
 */
export function parseAnkiFields(
	fieldsString: string,
	modelFields: AnkiModel["flds"]
 ...
 * 生成Weave默认牌组设置
 */
export function generateDefaultDeckSettings(): DeckSettings {
	const memoryScheduling = createDefaultMemorySchedulingSettings();
	return {
		newCardsPerDay: 20,
 ...
 * 创建默认FSRS卡片状态
 */
export function createDefaultFSRSCard(): FSRSCard {
 ...
 * 从Anki卡片数据创建FSRS状态
 */
export function createFSRSFromAnkiCard(ankiCard: APKGData["cards"][0]): FSRSCard {
	// Anki的due字段含义：
	...
	if (ankiCard.type === 0) {
		...
		// 学习卡片：due是时间戳
		dueDate = convertAnkiTimestamp(ankiCard.due);
	} else {
	...
	return {
		...
		state: mapCardState(ankiCard.type),
		lastReview: convertAnkiTimestamp(ankiCard.mod),
 ...
 */
export function createReviewLogFromAnki(ankiLog: APKGData["reviewLogs"][0]): ReviewLog {
	return {
		rating: mapRating(ankiLog.ease),
		state:
			ankiLog.type === 0
				? CardState.Learning
				: ankiLog.type === 1
				...
				: CardState.New,
		due: convertAnkiTimestamp(ankiLog.id, true), // revlog.id是时间戳
		...
		review: convertAnkiTimestamp(ankiLog.id, true),
	};
}
...
export const APKGMappingConfig = {
	convertAnkiTimestamp,
	mapCardType,
	mapCardState,
	mapRating,
	parseAnkiFields,
	applyFieldMapping,
	generateDefaultDeckSettings,
	createDefaultFSRSCard,
	createFSRSFromAnkiCard,
	createReviewLogFromAnki,
	cleanAnkiHTML,
	extractPlainText,
};
