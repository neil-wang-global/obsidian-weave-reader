...
type StateAction =
	| { type: ActionType.INITIALIZE; payload?: undefined; meta?: ActionMeta }
	| { type: ActionType.LOAD_CARD; payload: { card: Card }; meta?: ActionMeta }
	| { type: ActionType.LOAD_TEMPLATES; payload?: undefined; meta?: ActionMeta }
	| { type: ActionType.UPDATE_CONTENT; payload: { content: string }; meta?: ActionMeta }
	| {
			type: ActionType.UPDATE_FIELD;
			payload: { fieldKey: string; value: string };
			meta?: ActionMeta;
	  }
	| { type: ActionType.UPDATE_TAGS; payload: { tags: string[] }; meta?: ActionMeta }
	| { type: ActionType.UPDATE_DECK; payload: { deckId: string }; meta?: ActionMeta }
	| {
			type: ActionType.APPLY_FIELD_TEMPLATE;
			payload?: { template?: FieldTemplate | null };
			meta?: ActionMeta;
	  }
	| {
			type: ActionType.UPDATE_FIELDS_BULK;
			payload: {
				fields?: Record<string, string>;
				markdownContent?: string;
				templateId?: string;
	...
	| { type: ActionType.TOGGLE_PREVIEW; payload?: undefined; meta?: ActionMeta }
	| { type: ActionType.SWITCH_TAB; payload: { tab: UIState["activeTab"] }; meta?: ActionMeta }
	| { type: ActionType.TOGGLE_PANEL; payload?: undefined; meta?: ActionMeta }
	| { type: ActionType.SAVE_START; payload?: undefined; meta?: ActionMeta }
	| { type: ActionType.SAVE_SUCCESS; payload: { card: Card }; meta?: ActionMeta }
	| { type: ActionType.SAVE_ERROR; payload: { error: Error }; meta?: ActionMeta }
	| { type: ActionType.SET_ERROR; payload?: { errorMessage?: string }; meta?: ActionMeta }
	| { type: ActionType.CLEAR_ERROR; payload?: undefined; meta?: ActionMeta }
	| { type: ActionType.RECOVER; payload?: undefined; meta?: ActionMeta }
	| { type: ActionType.CLOSE; payload?: undefined; meta?: ActionMeta }
	| { type: ActionType.RESET; payload?: undefined; meta?: ActionMeta };

// 状态转换函数类型
type ActionFor<TType extends StateAction["type"]> = Extract<StateAction, { type: TType }>;

export type StateTransition<TType extends StateAction["type"] = StateAction["type"]> = (
	state: CardEditState,
	action: ActionFor<TType>
) => CardEditState;
...
export class CardEditStore {
	...
	constructor() {
		...
		this.hasError = derived(
			this.state,
			($state) => $state.phase === EditPhase.ERROR || !!$state.editorState.errorMessage
		);
	 ...
	 */
	addMiddleware(
		middleware: (action: StateAction, state: CardEditState) => StateAction | null
	): void {
	...
	private setupTransitions(): void {
		...
		this.addTransition(EditPhase.INITIALIZING, ActionType.LOAD_CARD, (state, action) => ({
			...
			editorState: {
				...
				isReady: true,
				hasContent: !!(
					action.payload.card.fields?.notes ||
					Object.keys(action.payload.card.fields || {}).length > 0
				),
			},
		}));
		...
		this.addTransition(EditPhase.READY, ActionType.UPDATE_FIELDS_BULK, (state, action) => ({
			...
			currentCard:
				action.payload.templateId && state.currentCard
					? {
							...state.currentCard,
	 ...
	 * 添加状态转换
	 */
	private addTransition<TType extends StateAction["type"]>(
		phase: EditPhase,
		actionType: TType,
		transition: StateTransition<TType>
	): void {
		const key = `${phase}-${actionType}`;
		this._transitions.set(key, transition as unknown as StateTransition);
	}
}
