...
export class SmartRetryService {
	...
	getRecoverySuggestions(error: Error): string[] {
		...
		switch (errorType) {
			case ErrorType.NETWORK:
				return ["检查网络连接", "稍后重试", "尝试刷新页面"];

			case ErrorType.DATA_CORRUPTION:
				return ["重新加载数据", "检查数据完整性", "联系技术支持"];

			case ErrorType.TIMEOUT:
				return ["减少数据范围", "稍后重试", "检查系统性能"];

			case ErrorType.PERMISSION:
				return ["检查权限设置", "重新登录", "联系管理员"];

			case ErrorType.CALCULATION:
				return ["检查输入数据", "重新计算", "使用默认参数"];

			default:
				return ["刷新页面", "重试操作", "联系技术支持"];
		}
	}
}
