...
function buildRootReadme(): string {
	return [
		...
		"可重建缓存、索引、日志和本机状态不在这里，而是在当前配置目录的 `plugins/<current-plugin-id>/` 下。",
		"",
		...
		"| 断点续学状态 | `当前配置目录/plugins/<current-plugin-id>/state/study-session.json` | 不在 `weave/` 内，不随 vault 数据目录一起展示 |",
		"| 插件本地缓存 / 索引 / 临时状态 | `当前配置目录/plugins/<current-plugin-id>/` | 属于插件本地状态，不是 vault 主数据 |",
		"",
		"## 非标准目录怎么理解",
		"",
