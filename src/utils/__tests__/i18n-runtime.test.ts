...
describe('i18n runtime fallbacks', () => {
  ...
  it('prefers concrete translations for high-value keys', () => {
    ...
    expect(i18n.t('views.epubSidebar.title')).toBe('E-book Sidebar');
    expect(i18n.t('views.epubBookshelfSidebar.title')).toBe('My Bookshelf');
    expect(i18n.t('views.epubView.menu.footnoteClickAction')).toBe('Footnote click action');
    expect(i18n.t('views.epubView.emptyState.openPremiumSettings')).toBe('Open license settings');
  });
