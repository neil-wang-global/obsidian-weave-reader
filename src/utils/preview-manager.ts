...
export class PreviewManager {
	 ...
	 * 渲染预览（带防抖和缓存）
	 */
	async renderPreview(content: string, container: HTMLElement): Promise<PreviewRenderResult> {
		...
		if (this.options.enableContentCache && this.contentCache.has(contentHash)) {
			...
			// /skip legacyHTML is used to restore cached Markdown render results from trusted internal source
			this.restoreCachedContent(container, cached);
		...
		return new Promise((resolve, reject) => {
			...
			// 注册定时器到资源管理器
			if (this.debounceTimer) {
				this.resourceManager.registerTimer(this.debounceTimer, "timeout", "预览防抖定时器");
			}
		});
	}
	...
	private async performRender(
		...
		try {
			...
			if (!content || content.trim() === "") {
				...
				/*
        // /skip legacyHTML is used with static trusted HTML string for empty content placeholder
        container.legacyHTML = '<div class="unified-preview-empty">预览内容为空</div>';
        */
			...
			if (this.options.enableContentCache) {
				// /skip reading legacyHTML to cache rendered Markdown output
				this.cacheRenderResult(
					contentHash,
					this.cloneContainerChildren(container),
			...
			logger.error("[PreviewManager] Render failed:", error);
			this.renderErrorState(container, error);
			/*
      logger.error(`[PreviewManager] 渲染失败 [${this.editorId}]:`, error);
      
      // 显示错误信息
      // /skip legacyHTML is used with trusted internal error message for preview error display
      container.legacyHTML = `
	...
	private cloneContainerChildren(container: HTMLElement): Node[] {
		return Array.from(container.childNodes, (node) => node.cloneNode(true));
	}

	private restoreCachedContent(container: HTMLElement, cached: CachedPreview): void {
		container.replaceChildren(...cached.renderedNodes.map((node) => node.cloneNode(true)));
	}
	 ...
	 * 缓存渲染结果
	 */
	private cacheRenderResult(contentHash: string, nodes: Node[], renderTime: number): void {
		// 清理过期缓存
