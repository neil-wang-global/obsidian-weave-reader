...
export class StudyQueueGenerator {
	 ...
	 * @returns 学习实例列表
	 */
	generateQueuePure(cards: Card[], onlyDue = true): StudyInstance[] {
	...
	filterQueue(
		...
		return queue.filter((_card) => {
			...
			if (filters.excludeNormalCards && !isProgressiveClozeChild(_card)) {
				return false;
			}
			...
			if (filters.includeTags && filters.includeTags.length > 0) {
				const hasRequiredTag = filters.includeTags.some((tag) => cardTags.includes(tag));
				if (!hasRequiredTag) {
					return false;
				}
			}
			...
			if (filters.excludeTags && filters.excludeTags.length > 0) {
				const hasExcludedTag = filters.excludeTags.some((tag) => cardTags.includes(tag));
				if (hasExcludedTag) {
					return false;
				}
			}
