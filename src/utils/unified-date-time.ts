...
export class UnifiedDateTimeProcessor {
	 ...
	 * 格式化相对时间
	 */
	formatRelativeTime(date: string | Date | number, config?: Partial<RelativeTimeConfig>): string {
		const dateObj = this.normalizeDate(date);
		if (!dateObj) return "-";
	...
	bucketDate(
		...
		bucketType: "day" | "week" | "month" | "year" = "day"
	): string {
		const dateObj = this.normalizeDate(date);
		if (!dateObj) return "";
	...
	private formatISODateTime(date: Date): string {
		return `${this.formatISODate(date)} ${this.pad2(date.getHours())}:${this.pad2(
			date.getMinutes()
	...
	private formatChineseDateTime(date: Date): string {
		return `${this.formatChineseDate(date)} ${this.pad2(date.getHours())}:${this.pad2(
			date.getMinutes()
		)}:${this.pad2(date.getSeconds())}`;
	}
 ...
 * 格式化ISO日期
 */
export const fmtISODate = (date: Date) => dateTimeProcessor.formatDate(date, DateFormat.ISO_DATE);
 ...
 * 获取一天开始时间
 */
export const startOfDay = (date: string | Date | number) => dateTimeProcessor.startOfDay(date);
...
export const bucketDate = (
	date: string | Date | number,
	bucketType?: "day" | "week" | "month" | "year"
) => dateTimeProcessor.bucketDate(date, bucketType);

/**
 * 解析日期
 */
export const parseDate = (dateString: string) => dateTimeProcessor.parseDate(dateString);
