...
import { normalizeQuestionType } from "../question-type-utils";
// 导入 YAML 工具函数
...
export function createTestCard(params: CreateTestCardParams): Card {
	...
	const normalizedQuestionType = normalizeQuestionType(params.questionType);
	...
	return {
		...
		// ✅ 可选的题型元数据（便于快速识别）
		metadata:
			normalizedQuestionType || params.correctAnswer
				? {
						...(normalizedQuestionType && { questionType: normalizedQuestionType }),
						questionMetadata: {
