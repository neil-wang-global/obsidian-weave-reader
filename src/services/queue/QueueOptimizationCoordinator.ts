...
export class QueueOptimizationCoordinator {
	 ...
	 * 计算所有卡片的优先级
	 */
	private calculatePriorities(cards: CardWithQueueOptimization[]): CardWithPriority[] {
	 ...
	 */
	private groupByPriorityLayer(items: CardWithPriority[]): CardWithPriority[][] {
		const layers: Map<number, CardWithPriority[]> = new Map();

		items.forEach((_item) => {
			// 按100分层
			const layerKey = Math.floor(_item.priority / 100);

			if (!layers.has(layerKey)) {
				layers.set(layerKey, []);
			}

			layers.get(layerKey)?.push(_item);
		});
	...
	private optimizeLayer(layer: CardWithPriority[]): CardWithQueueOptimization[] {
		...
		if (!this.settings.interleaving.enabled) {
			...
			return layer.sort((a, b) => a.priority - b.priority).map((item) => item.card);
		}
	...
	getQueueStatistics(cards: CardWithQueueOptimization[]): {
		...
		cards.forEach((_card) => {
			...
			// 统计Priority分布
			const priority = _card.priority || 2;
			stats.byPriority[priority] = (stats.byPriority[priority] || 0) + 1;

			// 累加优先级
			prioritySum += this.getPriority(_card);
		});
