...
export class PriorityCalculator {
	...
	private doCalculate(card: CardWithQueueOptimization): number {
		...
		// 第2层：用户优先级（0-40）
		const userPriority = this.config.enableUserPriority ? this.getUserPriority(card) : 20; // 默认值
		...
		// 第4层：Leech提升（-20 to 0）
		const leechBoost = this.config.enableLeechBoost ? this.getLeechBoost(card) : 0;
