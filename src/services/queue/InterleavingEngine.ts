...
export class InterleavingEngine {
	...
	private interleaveByTag(cards: CardWithQueueOptimization[]): CardWithQueueOptimization[] {
		...
		if (this.config.respectPriority) {
			groups.forEach((_group) => {
				_group.sort((a, b) => this.compareCards(a, b));
			});
		}
	...
	private smartInterleave(cards: CardWithQueueOptimization[]): CardWithQueueOptimization[] {
		...
		stateOrder.forEach((_state) => {
			const layer = layers.get(_state);
	...
	private groupByTag(cards: CardWithQueueOptimization[]): Map<string, CardWithQueueOptimization[]> {
		...
		cards.forEach((_card) => {
			const tag = this.getPrimaryTag(_card);

			if (!groups.has(tag)) {
				groups.set(tag, []);
			}

			groups.get(tag)?.push(_card);
		});
	...
	): Map<number, CardWithQueueOptimization[]> {
		const groups = new Map<number, CardWithQueueOptimization[]>();

		cards.forEach((_card) => {
			// 防御性检查
			if (!_card.fsrs) {
				logger.warn("[InterleavingEngine] Card missing fsrs data", { cardId: _card.uuid });
				return;
			}

			const state = _card.fsrs.state;

			if (!groups.has(state)) {
				groups.set(state, []);
			}

			groups.get(state)?.push(_card);
		});
	 ...
	 * 时间复杂度：O(n)
	 */
	private roundRobinInterleave(groups: CardWithQueueOptimization[][]): CardWithQueueOptimization[] {
		const result: CardWithQueueOptimization[] = [];
