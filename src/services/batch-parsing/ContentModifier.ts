...
import type { InsertionPlan, ModificationResult, PositionIndex } from "./types/sync-types";
...
export class ContentModifier {
	 ...
	 */
	async planInsertions(index: PositionIndex, uuidManager: UUIDManager): Promise<InsertionPlan[]> {
		...
		for (const card of index.cards) {
			...
			logDebugWithTag(
				"ContentModifier",
				`规划插入: UUID=${uuid}, BlockID=${blockId}, Pos=${insertPosition}`
			);
		}
	...
	applyInsertions(content: string, plans: InsertionPlan[]): ModificationResult {
		...
		for (let i = 0; i < plans.length; i++) {
			...
			logDebugWithTag(
				"ContentModifier",
				`已插入 ${i + 1}/${plans.length}: ${plan.content.length}字节 @ Pos=${plan.position}`
			);
		}
