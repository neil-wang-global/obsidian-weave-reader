...
export class IRPointStorageService {
	...
	private normalizePointFilesIndex(index: IRPointFileIndex | null | undefined): IRPointFileIndex {
		const files = Array.isArray(index?.files)
			? index.files
					...
					.map((entry) => {
						const relativePath = this.getNormalizedRelativePointFilePath(entry.file);
						return {
							topicId: String(entry.topicId || "").trim() || DEFAULT_TOPIC_ID,
							topicName: String(entry.topicName || "").trim() || DEFAULT_TOPIC_NAME,
							file:
								relativePath || `points/${this.buildTopicFileName(DEFAULT_TOPIC_NAME)}`,
	...
	async getPointFileEntryByPath(filePath: string): Promise<{
		...
		const root = this.getV2Paths().ir.root;
		...
		for (const entry of pointIndex.files || []) {
			...
			if (
				normalizedInput === absolutePath ||
				normalizedInput === relativePath ||
				normalizedInput.endsWith(`/${relativePath}`) ||
				normalizedInput.split("/").pop() === absolutePath.split("/").pop()
			) {
				return {
					topicId: String(entry.topicId || "").trim() || DEFAULT_TOPIC_ID,
					topicName: String(entry.topicName || "").trim() || DEFAULT_TOPIC_NAME,
		...
		const normalizedRootPrefix = `${normalizePath(root)}/`;
		const relativePath = normalizedInput.startsWith(normalizedRootPrefix)
			? normalizedInput.slice(normalizedRootPrefix.length)
			: this.getNormalizedRelativePointFilePath(normalizedInput.split("/").pop());
		...
		return {
			...
			relativePath: relativePath || normalizedInput.split("/").pop() || normalizedInput,
	...
	> {
		...
		for (const entry of index.files || []) {
			...
			const absolutePath = normalizePath(`${root}/${relativePath}`);
			if (seenFiles.has(absolutePath) || !(await this.adapter.exists(absolutePath))) {
				continue;
			}
		...
		if (await this.adapter.exists(pointsDir)) {
			...
			for (const file of listing.files || []) {
				...
				const isPointFile =
					fileName.endsWith(IR_DECK_FILE_EXTENSION) || LEGACY_POINT_FILE_PATTERN.test(fileName);
				if (!isPointFile || seenFiles.has(absolutePath)) {
					continue;
				}
				...
				const normalizedRootPrefix = `${normalizePath(root)}/`;
				const relativePath = absolutePath.startsWith(normalizedRootPrefix)
					? absolutePath.slice(normalizedRootPrefix.length)
					: this.getNormalizedRelativePointFilePath(fileName) || `points/${fileName}`;
	...
	async upsertPointDeck(deck: IRDeck): Promise<IRDeck> {
		...
		const previousAbsolutePath = existing
			? normalizePath(`${this.getV2Paths().ir.root}/${existing.file}`)
			: null;
	...
	private async buildMaterialCleanupPlan(): Promise<MaterialCleanupPlan> {
		...
		for (const entry of pointIndex.files || []) {
			...
			const filePath = normalizePath(`${this.getV2Paths().ir.root}/${entry.file}`);
	...
	private async applyMissingTargetPointCleanup(
		...
		const pointFilesIndex = await this.readPointFilesIndex();
		const rootPrefix = `${normalizePath(this.getV2Paths().ir.root)}/`;
		...
		for (const [filePath, group] of removalsByFile.entries()) {
			try {
				...
				const relativePath = normalizedFilePath.startsWith(rootPrefix)
					? normalizedFilePath.slice(rootPrefix.length)
					: this.getNormalizedRelativePointFilePath(normalizedFilePath);
				const indexEntry = (pointFilesIndex.files || []).find(
					(entry) =>
						this.getNormalizedRelativePointFilePath(entry.file) ===
						this.getNormalizedRelativePointFilePath(relativePath || normalizedFilePath)
				);
				if (indexEntry) {
	...
	async updatePointTopicIds(
		...
		for (const entry of index.files) {
			...
			const targetPreviousAbsolutePath = targetEntry
				? normalizePath(`${this.getV2Paths().ir.root}/${targetEntry.file}`)
				: null;
			const { relativePath, absolutePath: targetAbsolutePath } = await this.resolvePointFilePath(
				index,
				nextPrimaryTopicId,
	...
	private getNormalizedRelativePointFilePath(relativePath: string | null | undefined): string | null {
		const normalized = normalizePath(String(relativePath || "").trim());
		if (!normalized) {
			return null;
		}
		return normalized.startsWith("points/") ? normalized : `points/${normalized}`;
	}
	...
	private async resolveCanonicalPointFilePath(
		...
		while (true) {
			const relativePath = `points/${this.buildTopicFileName(topicName, ordinal)}`;
			const absolutePath = normalizePath(`${root}/${relativePath}`);
			const claimedByOther = (index.files || []).some(
				(entry) =>
					String(entry?.topicId || "").trim() !== topicId &&
					this.getNormalizedRelativePointFilePath(entry?.file) === relativePath
			);
	...
	private async normalizePointFileNaming(): Promise<void> {
		...
		for (const entry of index.files) {
			...
			const previousRelativePath = this.getNormalizedRelativePointFilePath(entry.file);
			const previousAbsolutePath = previousRelativePath
				? normalizePath(`${this.getV2Paths().ir.root}/${previousRelativePath}`)
				: null;
	...
	private async resolvePointFilePath(
		...
		if (existing) {
			if (existing.file !== canonical.relativePath) {
				existing.file = canonical.relativePath;
				existing.topicName = topicName;
			}
			return canonical;
		}
	...
	private async collectMigratedPointIds(): Promise<Set<string>> {
		...
		for (const entry of index.files || []) {
			...
			const filePath = normalizePath(`${root}/${relativePath}`);
		...
		if (await this.adapter.exists(pointsDir)) {
			...
			for (const file of listing.files || []) {
				...
				const isPointFile =
					fileName.endsWith(IR_DECK_FILE_EXTENSION) || LEGACY_POINT_FILE_PATTERN.test(fileName);
				if (!isPointFile || seenFiles.has(filePath)) {
					continue;
				}
				seenFiles.add(filePath);
				pointFiles.push({
