...
export class AnchorManager {
	...
	parseAnchorsFromContent(content: string): AnchorParseResult[] {
		const anchors: AnchorParseResult[] = [];
	 ...
	 * @returns 锚点列表
	 */
	async parseAnchorsFromFile(file: TFile): Promise<AnchorParseResult[]> {
		try {
			const content = await this.app.vault.read(file);
	 ...
	 */
	async getLatestAnchor(file: TFile): Promise<AnchorParseResult | null> {
	 ...
	 * @param anchorId 锚点ID
	 */
	async jumpToAnchor(file: TFile, anchorId: string): Promise<boolean> {
		try {
			const content = await this.app.vault.read(file);
	 ...
	 * @param anchorId 锚点ID
	 */
	async removeAnchor(file: TFile, anchorId: string): Promise<boolean> {
		try {
			const content = await this.app.vault.read(file);
	 ...
	 * 查找锚点在内容中的位置
	 */
	private findAnchorPosition(content: string, anchorId: string): number {
