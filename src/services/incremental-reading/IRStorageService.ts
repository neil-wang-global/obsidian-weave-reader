...
export class IRStorageService {
	...
	private getReadableRoots(): { legacyRoot: string; currentRoot: string } | null {
		try {
			const parentFolder = normalizeWeaveParentFolder(getCompatibleWeaveParentFolder(this.app));
			const irSettings = getCompatibleIncrementalReadingSettings(this.app);
			let currentRoot = normalizePath(getReadableWeaveRoot(parentFolder));

			if (!parentFolder) {
				const importFolder = irSettings.importFolder;
