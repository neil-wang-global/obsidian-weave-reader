...
export class IRScheduleKernel {
	...
	private cognitiveProfileService: IRCognitiveProfileService;
	...
	constructor(app: App) {
		...
		this.cognitiveProfileService = new IRCognitiveProfileService();
		this.planGenerator = new IRPlanGeneratorService(this.cognitiveProfileService);
	}
	...
	private getReadingMaterialStorage(): ReadingMaterialStorage {
		if (!this.readingMaterialStorage) {
			this.readingMaterialStorage = new ReadingMaterialStorage(this.app);
		}
		return this.readingMaterialStorage;
	}
	...
	private getPlanningSettingsSnapshot(): IRAdvancedScheduleSettings {
		...
		try {
			...
			return {
				...
				interleaveMode:
					typeof ir?.interleaveMode === "boolean"
						? ir.interleaveMode
	...
	private async getReadingMaterials(): Promise<ReadingMaterial[]> {
		try {
			const storage = this.getReadingMaterialStorage();
		...
		const manager = getCompatibleReadingMaterialManager(this.app);
		...
		try {
			...
			logger.warn("[IRScheduleKernel] 读取兼容阅读材料失败:", error);
			return [];
		}
	}
	...
	private async getAssociatedNoteSignalIndex(): Promise<IRAssociatedNoteSignalIndex> {
		...
		if (dataStorage?.getAllCards) {
			try {
				const cards = (await dataStorage.getAllCards()) as Card[];
