...
import { getCompatibleIncrementalReadingSettings } from "../../utils/plugin-access";
...
export class IRV4SchedulerService {
	...
	private getAdvancedSettingsSnapshot(): IRAdvancedScheduleSettings {
		...
		try {
			const ir = getCompatibleIncrementalReadingSettings(this.app);

			const enableTagGroupPrior = ir?.enableTagGroupPrior ?? defaults.enableTagGroupPrior;
			const rawInterleaveMode = ir?.interleaveMode as unknown;
			const rawAgingStrength = ir?.agingStrength as unknown;
			const rawAutoPostponeStrategy = ir?.autoPostponeStrategy as unknown;
			const interleaveMode =
				typeof rawInterleaveMode === "boolean"
					? rawInterleaveMode
					: defaults.interleaveMode;
			const agingStrength =
				rawAgingStrength === "low" ||
				rawAgingStrength === "medium" ||
				rawAgingStrength === "high"
					? rawAgingStrength
					: defaults.agingStrength;
			const autoPostponeStrategy =
				rawAutoPostponeStrategy === "off" ||
				rawAutoPostponeStrategy === "gentle" ||
				rawAutoPostponeStrategy === "aggressive"
					? rawAutoPostponeStrategy
					: defaults.autoPostponeStrategy;
	...
	private getTagGroupFollowMode(): "off" | "ask" | "auto" {
		try {
			return getCompatibleIncrementalReadingSettings(this.app).tagGroupFollowMode ?? "ask";
		} catch {
			return "ask";
		}
	}
