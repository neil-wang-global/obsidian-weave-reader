...
import {
	...
	getCompatibleSelectionQuickCreateLastFolder,
...
export class ReadingMaterialManager {
	...
	private getDefaultImportFolderFromPluginSettings(): string {
		try {
			const importFolder = getCompatibleIncrementalReadingSettings(this.app).importFolder;
			const parentFolder = getCompatibleWeaveParentFolder(this.app);
	...
	private getSelectionQuickCreateLastFolderFromPluginSettings(): string {
		try {
			return getCompatibleSelectionQuickCreateLastFolder(this.app);
		} catch {
			return "";
		}
	}
