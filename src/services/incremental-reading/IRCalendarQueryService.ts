...
export class IRCalendarQueryService {
	...
	private memoryCache = new Map<string, IRCalendarQueryCacheEntry>();
	...
	constructor(app: App) {
		this.app = app;
		this.diskCachePath = getPluginPaths(app).cache.incrementalReading.irCalendarCache;
	}
	...
	private async getReadingMaterials(): Promise<ReadingMaterial[]> {
		...
		try {
			...
			logger.warn("[IRCalendarQueryService] 读取兼容阅读材料失败", error);
			return [];
		}
	}
	...
	private buildSettingsFingerprint(): string {
		return this.hashStableValue(getCompatibleIncrementalReadingSettings(this.app));
	}
