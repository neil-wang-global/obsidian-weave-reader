...
export class QuickCreateCleanupStrategy implements ICleanupStrategy {
	...
	async execute(target: CleanupTarget, vault: Vault): Promise<CleanupResult> {
		...
		try {
			...
			// 移除块链接
			const cleanedContent = await this.removeBlockLinkFromLine(content, target.blockId!);
			...
			if (cleanedContent !== content) {
				...
				logDebugWithTag(
					"QuickCreateCleanup",
					`清理块链接: ^${target.blockId} from ${target.filePath}`
				);
			}
	 ...
	 * @returns 清理后的内容
	 */
	private async removeBlockLinkFromLine(content: string, blockId: string): Promise<string> {
		...
		// 重新组装内容
		const result =
			content.substring(0, lineStartIndex) + cleanedLine + content.substring(actualLineEnd);

		logDebugWithTag(
			"QuickCreateCleanup",
			`清理成功: ^${blockId}, 行: "${currentLine}" -> "${cleanedLine}"`
		);
