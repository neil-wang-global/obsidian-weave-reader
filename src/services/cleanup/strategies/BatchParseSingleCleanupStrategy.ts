...
export class BatchParseSingleCleanupStrategy implements ICleanupStrategy {
	 ...
	 * @returns 清理后的内容
	 */
	private async removeBlockLink(content: string, blockId: string): Promise<string> {
	...
	private mergeDeletionTag(existingTags: unknown): string[] | null {
		...
		const hasDeletionTag = normalizedTags.some(
			(tag) => tag.replace(/^#/, "").toLowerCase() === this.DELETION_TAG.toLowerCase()
		);
		if (hasDeletionTag) {
			return null;
		}
