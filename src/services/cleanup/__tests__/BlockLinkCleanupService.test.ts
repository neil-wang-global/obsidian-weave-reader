...
describe('BlockLinkCleanupService', () => {
  ...
  it('batch-cleans multiple quick-create block links in one file without touching other lines', async () => {
    currentContent = [
      'line a ^we-quick001',
      'line b ^we-quick002',
      'line c ^we-keep003'
    ...
    const results = await service.cleanupAfterCardDeletions([
      {
        uuid: 'tk-quick000001',
        sourceFile: 'mixed.md',
        sourceBlock: '^we-quick001',
        isBatchScanned: false,
        content: '',
        tags: []
      } as any,
    ...
    expect(currentContent).toContain('line a');
    expect(currentContent).toContain('line b');
    expect(currentContent).not.toContain('^we-quick001');
    expect(currentContent).not.toContain('^we-quick002');
    expect(currentContent).toContain('line c ^we-keep003');
  });

  it('batch-cleans multiple batch-multi cards in one file without touching unrelated blocks', async () => {
    currentContent = [
      '<->',
      'alpha question',
      '---div---',
      'alpha answer',
      '<!-- tk-alpha0000001 --> ^we-alpha001',
      '<->',
      '',
      '<->',
      'beta question',
    ...
    const results = await service.cleanupAfterCardDeletions([
      {
        uuid: 'tk-alpha0000001',
        sourceFile: 'mixed.md',
        sourceBlock: '^we-alpha001',
        isBatchScanned: true,
        content: '',
        tags: []
      } as any,
      {
        uuid: 'tk-beta00000002',
        sourceFile: 'mixed.md',
        sourceBlock: '^we-beta002',
        isBatchScanned: true,
        content: '',
        tags: []
      } as any
    ...
    expect(results).toHaveLength(1);
    expect(results[0].cleanedItems.length).toBe(2);
    expect(currentContent).not.toContain('<!-- tk-alpha0000001 --> ^we-alpha001');
    expect(currentContent).not.toContain('<!-- tk-beta00000002 --> ^we-beta002');
    expect(currentContent).toContain('<!-- tk-gamma0000003 --> ^we-gamma003');
    expect((currentContent.match(/#we_已删除/g) || []).length).toBe(2);
  });
});
