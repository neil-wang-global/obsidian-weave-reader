...
export class CardFileService {
	...
	private async withShardLock<T>(fileName: string, fn: () => Promise<T>): Promise<T> {
		...
		let resolve: () => void = () => {};
		const next = new Promise<void>((_r) => {
			resolve = _r;
		});
	...
	private async rebuildIndexFromDisk(): Promise<void> {
		...
		const listing = adapter.list
			? await adapter.list(this.cardsFolder)
			: { files: [], folders: [] };
	...
	async deleteCardsBatch(uuids: string[]): Promise<{ deleted: string[]; notFound: string[] }> {
		...
		try {
			...
			logger.info(
				`[CardFileService] 批量删除完成: 成功 ${deleted.length}, 未找到 ${notFound.length}`
			);
			return { deleted, notFound };
		} catch (error) {
	...
	/**
	 * 按 UUID 批量获取卡片（高效：按分片分组，每个分片只读取一次）
	 */
	async getCardsByUUIDsBatch(uuids: string[]): Promise<{ found: Card[]; notFound: string[] }> {
		const found: Card[] = [];
		const notFound: string[] = [];
		...
		try {
			...
			const foundMap = new Map<string, Card>();
			...
			logger.debug(
				`[CardFileService] 批量读取卡片完成: 找到 ${found.length}, 未找到 ${notFound.length}`
			);
			return { found, notFound };
		} catch (error) {
			logger.error("[CardFileService] 批量读取卡片失败:", error);
			return { found, notFound };
		}
	}
	...
	private async checkAndAutoShard(fileName: string): Promise<void> {
		...
		// 需要分片
		logger.info(
			`[CardFileService] 文件 ${fileName} 需要分片 (${fileInfo.cardCount} 卡片, ${
				stat?.size ?? 0
			} 字节)`
		);
	...
	private async performAutoShard(fileName: string, index: CardFileIndex): Promise<void> {
		...
		index.files.push({
			...
			displayName: `${
				index.files.find((f) => f.fileName === fileName)?.displayName || fileName
			} (分片${nextShardNum})`,
	...
	private async shardOversizedFiles(): Promise<void> {
		try {
			...
			for (const fileInfo of [...index.files]) {
				...
				if (countExceeded || sizeExceeded) {
					logger.info(
						`[CardFileService] 启动分片: ${fileInfo.fileName} (${fileInfo.cardCount} 卡片, ${stat.size} 字节)`
					);
	...
	private async migrateOldFileNames(adapter: any): Promise<void> {
		try {
					...
					if (await adapter.exists(newPath)) {
						...
						try {
							...
							if (Array.isArray(oldData?.cards) && Array.isArray(newData?.cards)) {
								...
								for (const c of newData.cards) {
									if (c?.uuid) merged.set(c.uuid, c);
								}
								for (const c of oldData.cards) {
									if (c?.uuid && !merged.has(c.uuid)) merged.set(c.uuid, c);
								}
								await adapter.write(
									newPath,
									JSON.stringify({ cards: Array.from(merged.values()) })
								);
							}
			...
			const nonStandardFiles = jsonFiles.filter(
				(f) =>
			...
			if (nonStandardFiles.length > 0) {
				...
				const primaryCards = new Map<string, any>();
					...
					if (await adapter.exists(primaryPath)) {
						...
						if (Array.isArray(data?.cards)) {
							for (const c of data.cards) {
								if (c?.uuid) primaryCards.set(c.uuid, c);
							}
						}
					}
				} catch {}
				...
				if (primaryCards.size > 0) {
					await adapter.write(
						primaryPath,
						JSON.stringify({ cards: Array.from(primaryCards.values()) })
					);
				}
			}
