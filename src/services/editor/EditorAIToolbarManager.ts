...
export class EditorAIToolbarManager {
	...
	private activeToolbar: ToolbarInstance | null = null;
	 ...
	 */
	private async show(view: MarkdownView, leafId: string): Promise<void> {
		try {
			// 获取当前文档内容
			...
			// 动态导入并挂载 Svelte 组件
			const { default: EditorAICardToolbar } = await import(
				"../../components/ai-assistant/EditorAICardToolbar.svelte"
			);
	...
	close(): void {
		...
		try {
			void unmount(this.activeToolbar.component);
