...
describe('editor-temp-file-policy', () => {
  ...
  it('resolves detached editor temp folders from sourcePath and plugin configDir', () => {
    ...
    expect(getPluginEditorTempDir(defaultApp)).toBe('.obsidian/plugins/weave-epub-reader/cache/editor-temp');
    expect(getPluginEditorTempDir(customApp)).toBe('custom-config/plugins/weave-epub-reader/cache/editor-temp');
  ...
  it('distinguishes plugin cache modal buffers from legacy modal buffers', () => {
    ...
    expect(
      isPluginCacheModalEditorPermanentFilePath(
        app,
        '.obsidian/plugins/weave-epub-reader/cache/editor-temp/modal-editor-permanent-2.md',
      ),
    ).toBe(true);
