...
import { CardCreationProcessor, type CardCreationResult } from "./CardCreationProcessor";
...
export class ProgressiveClozeCardManager {
	private processor: CardCreationProcessor;

	constructor(private storageService?: ICardStorageService) {
	 ...
	 * @returns 创建结果
	 */
	async createCard(card: Card, options: CardCreationOptions = {}): Promise<CardCreationResult> {
	...
	async createCards(
		...
		logger.info(
			`[ProgressiveClozeCardManager] Processed ${successCount}/${cards.length} cards, ` +
	 ...
	 * @returns 更新是否成功
	 */
	async updateParentCard(parentCard: ProgressiveClozeParentCard): Promise<boolean> {
	 ...
	 * @returns 删除是否成功
	 */
	async deleteProgressiveCloze(parentCardId: string): Promise<boolean> {
		if (!this.storageService) {
			throw new Error("Storage service not provided");
		}
		...
		try {
			...
			logger.info(
				`[ProgressiveClozeCardManager] Deleted progressive cloze: 1 parent + ${childCardIds.length} children`
			);
	 ...
	 * @returns 删除是否成功
	 */
	async deleteChildCard(childCardId: string): Promise<boolean> {
		if (!this.storageService) {
			throw new Error("Storage service not provided");
		}
		...
		try {
			...
			// 3. 更新父卡片的子卡片列表
			const updatedChildIds = parentCard.progressiveCloze.childCardIds.filter(
				(_id) => _id !== childCardId
			);
			...
			if (updatedChildIds.length < 2) {
				logger.warn(
					`[ProgressiveClozeCardManager] Only ${updatedChildIds.length} child card(s) remaining, should consider downgrading to normal card`
				);
				// TODO: 实现降级逻辑
			}
	...
	getChildCards(parentCardId: string): ProgressiveClozeChildCard[] {
		...
		return cards.filter((c): c is ProgressiveClozeChildCard => isProgressiveClozeChild(c));
	}
}
