 ...
 * 颜色缓存（优化性能）
 */
const colorCache = new Map<string, { rgb: string; opacity: number }>();
 ...
 */
export function parseRGBAColor(color: string): { rgb: string; opacity: number } {
	// 检查缓存
 ...
 * @returns RGBA字符串
 */
export function createRGBAColor(rgb: string, opacity: number): string {
	// 如果rgb已经是rgb()格式，提取数值
 ...
 * @returns 限制后的遮罩
 */
export function clampMask(mask: Mask): Mask {
	if (mask.type === "rect") {
		return {
			...mask,
 ...
 */
export function getRectBounds(mask: Mask): {
	left: number;
	top: number;
	right: number;
	bottom: number;
 ...
 * @returns 是否在遮罩内
 */
export function isPointInMask(point: { x: number; y: number }, mask: Mask): boolean {
	if (mask.type === "rect") {
 ...
 * @returns 是否碰撞
 */
export function checkMaskCollision(mask1: Mask, mask2: Mask): boolean {
	// 矩形 vs 矩形
 ...
 */
export function findMaskAtPoint(point: { x: number; y: number }, masks: Mask[]): Mask | null {
	// 从后往前查找（后面的遮罩在上层）
 ...
 * @returns 控制点位置数组
 */
export function getRectResizeHandles(mask: Mask): Array<{
	type: ResizeHandle;
	x: number;
	y: number;
}> {
 ...
 */
export function getCircleResizeHandles(mask: Mask): Array<{
 ...
 * @returns 是否有效
 */
export function isValidMask(mask: Partial<Mask>): boolean {
 ...
 * @returns 是否太小
 */
export function isMaskTooSmall(mask: Mask, threshold = 0.01): boolean {
	if (mask.type === "rect") {
