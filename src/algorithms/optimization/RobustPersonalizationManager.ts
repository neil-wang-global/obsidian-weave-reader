...
export class RobustPersonalizationManager extends PersonalizationManager {
	...
	async updateAfterReview(review: ReviewLog, allHistory: ReviewLog[]): Promise<void> {
		...
		const backtrackedWeights = await this.backtracking.detectAndBacktrack(currentPerformance);
	...
	private async createPerformanceCheckpoint(history: ReviewLog[]): Promise<void> {
		...
		this.backtracking.createCheckpoint(reviewCount, currentWeights, performance);
	...
	private async handleBacktracking(
		...
		// 显示通知
		new Notice(
			`📉 检测到学习效果波动，已自动优化参数（衰减因子: ${(decayFactor * 100).toFixed(0)}%）`,
			4000
		);
	...
	protected async applyOptimizedWeights(weights: number[]): Promise<void> {
		try {
			// 通过父类的 plugin.app 保存
			vaultStorage.setItem("weave_optimized_weights_default", JSON.stringify(weights));
