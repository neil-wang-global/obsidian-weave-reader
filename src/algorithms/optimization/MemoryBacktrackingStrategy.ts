...
export class MemoryBacktrackingStrategy {
	 ...
	 * 创建检查点（每次参数更新时）
	 */
	createCheckpoint(reviewCount: number, weights: number[], performance: PerformanceMetrics): void {
	 ...
	 * 检测性能下降并回溯
	 */
	async detectAndBacktrack(currentPerformance: PerformanceMetrics): Promise<number[] | null> {
		// 获取最近的检查点
		const checkpoints = Array.from(this.historyCheckpoints.values()).sort(
		...
		if (performanceDrop > this.performanceThreshold) {
			logger.warn(
				`⚠️ [BacktrackingStrategy] 检测到性能下降 ${(performanceDrop * 100).toFixed(1)}%，执行回溯`
			);
	...
	private calculateStabilityScore(performance: PerformanceMetrics): number {
		// 加权组合：准确率70% + 保持率30%
		return performance.predictionAccuracy * 0.7 + performance.retentionRate * 0.3;
	}
	...
	getCheckpointHistory(): WeightCheckpoint[] {
		return Array.from(this.historyCheckpoints.values()).sort(
