...
import type { AnkiModel, AnkiTemplate } from "../../../domain/apkg/types";
...
interface AnkiImportedParseTemplate extends ParseTemplate {
	ankiImportMetadata: AnkiImportMetadata;
}

function isAnkiImportedParseTemplate(
	template: ParseTemplate
): template is AnkiImportedParseTemplate {
	const candidate = template as Partial<AnkiImportedParseTemplate>;
	return typeof candidate.ankiImportMetadata?.originalAnkiModelId === "string";
}
...
export class AnkiTemplateConverter {
	...
	convertModelToTemplate(
		...
		this.logger.debug(
			`转换了 ${templateFields.length} 个字段:`,
			templateFields.map((f) => `${f.name}(${f.side})`).join(", ")
		);

		// 创建ParseTemplate（带Anki扩展元数据）
		const template: AnkiImportedParseTemplate = {
	 ...
	 */
	findExistingTemplate(modelId: number, existingTemplates: ParseTemplate[]): ParseTemplate | null {
		const existing = existingTemplates.find(
			(template) =>
				isAnkiImportedParseTemplate(template) &&
				template.ankiImportMetadata.originalAnkiModelId === modelId.toString()
		);
