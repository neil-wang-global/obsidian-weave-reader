<script lang="ts">
  ...
  type DataSource = 'memory' | 'questionBank' | 'incremental-reading' | 'bookshelf' | 'epub-highlights';
  ...
  let usesUnifiedSearchShell = $derived(
    dataSource === 'bookshelf' || dataSource === 'epub-highlights'
  );
  ...
  const dataSourceOptions = $derived.by(() => {
    if (dataSource === 'bookshelf') {
      return [...bookshelfSearchOptions];
    }

    if (dataSource === 'epub-highlights') {
      return [...epubHighlightSearchOptions];
    }
  ...
  <div class="search-input-wrapper" class:is-unified-shell={usesUnifiedSearchShell}>
    ...
    {#if showSortButton}
      <div
        ...
        aria-label={t('epub.globalSidebar.searchUi.sort')}
        title={t('epub.globalSidebar.searchUi.sortOptions')}
      >
