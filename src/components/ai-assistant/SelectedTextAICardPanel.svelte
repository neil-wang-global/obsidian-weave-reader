<script lang="ts">
  ...
  import { findVisibleSplitActionById } from '../../services/ai/ai-action-config';
  ...
  let currentAction = $derived.by(() => {
    return findVisibleSplitActionById(host, actionId);
  });
...
<div class="weave-ai-card-panel">
  <div class="header">
    ...
    <div class="header-actions">
      <button
        ...
        type="button"
        title="按住并上下拖动，调节预览区高度"
        aria-label="按住并上下拖动，调节预览区高度"
  ...
  <div class="content" style={`height: ${previewHeight}px; --weave-ai-preview-card-height: ${Math.max(156, previewHeight - 28)}px;`}>
    {#if isGenerating}
