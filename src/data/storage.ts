...
import { getCompatibleWeaveParentFolderFromSettingsOwner } from "../utils/plugin-access";
...
export class WeaveDataStorage {
	...
	private get v2Paths() {
		const parentFolder = normalizeWeaveParentFolder(
			getCompatibleWeaveParentFolderFromSettingsOwner(this.plugin as any)
		);
	...
	private async ensurePersistedDeckUsesWDeck(deck: Deck): Promise<Deck> {
		if (
			...
			deck.purpose === "test" ||
			typeof (this.plugin.wdeckService as any).ensureDeckFileForDeck !== "function"
		) {
			return deck;
		}
	...
	private async readAllCardsFromUnifiedStorage(): Promise<Card[]> {
		...
		try {
			...
			logger.warn("[Storage] 统一存储读取失败:", error);
			return [];
		}
	}
	...
	private async getExistingCardUUIDSet(uuids: string[]): Promise<Set<string>> {
		...
		try {
			if (typeof cardFileService.getCardsByUUIDsBatch === "function") {
				const result = await cardFileService.getCardsByUUIDsBatch(uniqueUUIDs);
			...
			logger.warn("[Storage] 检查既有卡片失败:", error);
		}
	...
	private async getCardsForDeck(deckId: string): Promise<Card[]> {
		...
		if (!hasFormalBindings && !isEmergentDeckId && indexService && typeof cardFileService?.getCardsByUUIDsBatch === "function") {
			try {
				...
				if (
					...
					const batchResult = await cardFileService.getCardsByUUIDsBatch(deckState.cardUUIDs);
					const indexedCards = (batchResult?.found || []).map((card: Card) =>
						this.hydrateCardFromYAML(card)
					);
					const yamlBackedCards = filterCardsByDeck(indexedCards, { fallbackToReferences: false });
		...
		const allCards = await this.readAllCardsFromUnifiedStorage();
	...
	async saveDeck(deck: Deck): Promise<ApiResponse<Deck>> {
		try {
			...
			if (this.plugin.wdeckService && normalizedDeck.purpose !== "test") {
				...
				const savedAggregate = await (this.plugin.wdeckService as any).saveDeckDefinition({
					...normalizedDeck,
					id: logicalDeckId,
				...
				const dataChangeContext = (this.plugin as any).__weaveDataChangeContext;
				if (
					dataSyncService &&
					typeof dataSyncService.notifyChange === "function" &&
					!dataChangeContext?.suppressDeckNotifications
				) {
					await dataSyncService.notifyChange({
						...
						ids: [savedDeck.id],
						metadata: {
							source: dataChangeContext?.source,
							deckIds:
		...
		try {
			if (this.plugin.wdeckService?.isWDeckDeckId(deckId)) {
				if (options?.skipCardDeletion) {
					if (typeof (this.plugin.wdeckService as any).dissolveDeckByDeckId !== "function") {
						return {
							success: false,
							error: "WDeck 牌组当前不支持保留卡片的解散操作。",
							timestamp: new Date().toISOString(),
						};
					}
					await (this.plugin.wdeckService as any).dissolveDeckByDeckId(deckId);
				} else {
	...
	private async saveCardInternal(
			...
			try {
				...
				if (dataSyncService && typeof dataSyncService.notifyChange === "function") {
					await dataSyncService.notifyChange({
						...
						metadata: {
							...(dataChangeContext?.source ? { source: dataChangeContext.source } : {}),
		...
		if (
			this.plugin.wdeckService &&
			cardWithYAML.cardPurpose !== "test" &&
			typeof (this.plugin.wdeckService as any).saveCardToDeck === "function"
		) {
			try {
	...
	private async saveCardsInternalBatch(
		...
		try {
			...
			const { isValidUUID } = await import("../utils/helpers");
			const now = new Date();
			const cardsAfterGateway = (await gateway.processBatch(normalizedCards)).map((c) =>
				this.normalizeCardData(c)
			);
			...
			const dataSyncService = (this.plugin as any).dataSyncService;
			if (
				cardsToSave.length + wdeckCardsToSave.length + savedDeckBoundCards.length > 0 &&
				dataSyncService &&
				typeof dataSyncService.notifyChange === "function"
			) {
		...
		if (this.plugin.cardFileService || this.plugin.wdeckService) {
			try {
				...
				for (const uuid of deletedBatchUUIDs) {
					...
					if (this.plugin.cardIndexService) {
						this.plugin.cardIndexService.removeCardIndex(card.uuid);
					}
	...
	async persistAllDeckStats(statsMap: Record<string, Partial<DeckStats>>): Promise<void> {
		try {
			...
			if (changedDecks.length > 0) {
				const previousDataChangeContext = (this.plugin as any).__weaveDataChangeContext;
				(this.plugin as any).__weaveDataChangeContext = {
				...
				try {
					...
					(this.plugin as any).__weaveDataChangeContext = previousDataChangeContext;
				}
	...
	async saveDeckCards(deckId: string, cards: Card[]): Promise<void> {
		...
		const targetUUIDSet = new Set(cards.map((card) => card.uuid));
		const cardsToSave: Card[] = [];
		...
		for (const card of cards) {
			...
			cardsToSave.push({
				...syncedCard,
				created: syncedCard.created || now.toISOString(),
				modified: now.toISOString(),
			});
		}

		for (const existingCard of existingDeckCards) {
			if (targetUUIDSet.has(existingCard.uuid)) {
				continue;
			}
			...
			if (remainingDeckIds.length > 0) {
				nextCard.deckId = remainingDeckIds[0];
			} else {
				(nextCard as Partial<Card>).deckId = undefined;
			}

			const syncedCard = await this.syncCardMetadataToYAML(nextCard, {
				syncDeckMembershipFromRuntime: true,
			});
		...
		if (cardsToSave.length > 0) {
			...
			if (!ok) {
				throw new Error("CardFileService 未初始化或保存失败");
			}

			if (this.plugin.cardMetadataCache) {
				for (const card of cardsToSave) {
					this.plugin.cardMetadataCache.invalidate(card.uuid);
				}
			}
		}
	}
